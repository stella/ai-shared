...
GENERATED_CLAUDE_DIR="$TMP_ROOT/.claude/skills"
...
TARGET_CLAUDE_DIR="$REPO_ROOT/.claude/skills"
LEGACY_CLAUDE_COMMANDS_DIR="$REPO_ROOT/.claude/commands"
...
sync_from_source() {
  ...
  find "$source_dir" -mindepth 1 -maxdepth 1 | sort | while read -r entry_path; do
    ...
    copy_skill_dir "$source_dir" "$entry" "$skill_name" "$claude_dir"
    copy_skill_dir "$source_dir" "$entry" "$skill_name" "$agents_dir"
  done
}
...
# Keep this file so the directory exists even when no skills are synced yet.
EOF
}
...
const generatedMarker =
  "<!-- Generated by .ai/shared/scripts/sync-ai-skills.sh from .ai/manifest.json. -->";
const legacyRootClaudeAliases = new Set([
  [
    "@AGENTS.md",
    "",
    "## Claude Code",
...
for (const stalePath of readRegistry(targetRegistry)) {
  if (!desiredSet.has(stalePath)) {
    ...
    if (existing?.isSymbolicLink()) {
      ...
      const isLegacyRootClaudeAlias =
        stalePath === "CLAUDE.md" && legacyRootClaudeAliases.has(contents);
      if (!contents.includes(generatedMarker) && !isLegacyRootClaudeAlias) {
        throw new Error(
...
# Every backticked `/name` in a generated prompt or skill is a skill invocation
# (`/conventions-db`, `/rabbit-round`). A reference to a skill this repository
# does not generate is silent drift: agents are told to read something that does
# not exist. Fail instead.
check_skill_references() {
  local errors=0 reference name
  local -a skill_names=()

  while IFS= read -r name; do
    [ -n "$name" ] || continue
    skill_names+=("$name")
  done < <(find "$GENERATED_AGENTS_DIR" -mindepth 1 -maxdepth 1 -type d -exec basename {} \; | sort)

  while IFS= read -r reference; do
    [ -n "$reference" ] || continue
    name="${reference#\`/}"
    name="${name%\`}"
    if ! printf '%s\n' "${skill_names[@]}" | grep -qx -- "$name"; then
      echo "error: prompt references \`/$name\`, which is not a generated skill" >&2
      errors=$((errors + 1))
    fi
  done < <(
    { [ -d "$GENERATED_PROMPTS_DIR" ] && find "$GENERATED_PROMPTS_DIR" -type f -name '*.md'; \
      find "$GENERATED_AGENTS_DIR" -type f -name 'SKILL.md'; } |
      xargs grep -hoE '`/[A-Za-z0-9_-]+`' 2>/dev/null | sort -u
  )

  return "$errors"
}

# `.claude/commands` was a generated target before skills moved to
# `.claude/skills`. Sync owned that directory wholesale, so removing it is the
# cutover; a leftover copy would expose every skill twice.
check_legacy_claude_commands() {
  [ -e "$LEGACY_CLAUDE_COMMANDS_DIR" ] || return 0

  echo "error: $LEGACY_CLAUDE_COMMANDS_DIR is a legacy generated directory. Run: bun run sync-ai" >&2
  return 1
}
...
write_skill_files
write_shared_root_files
check_skill_references || exit 1

if [ "$CHECK_MODE" = true ]; then
  errors=0
  root_file=""

  check_legacy_claude_commands || errors=$((errors + 1))
...
rm -rf "$TARGET_CLAUDE_DIR" "$TARGET_AGENTS_DIR" "$LEGACY_CLAUDE_COMMANDS_DIR"
